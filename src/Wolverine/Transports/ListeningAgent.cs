        ...
        if (endpoint.CircuitBreakerOptions != null)
        {
            ...
            _pipeline = new HandlerPipeline(runtime,
                new CircuitBreakerTrackedExecutorFactory(_circuitBreaker,
                    new CircuitBreakerTrackedExecutorFactory(_circuitBreaker, runtime)), endpoint){TelemetryEnabled = endpoint.TelemetryEnabled};
        }
        else
        {
            _pipeline = new HandlerPipeline(runtime, runtime, endpoint){TelemetryEnabled = endpoint.TelemetryEnabled};
        }
